...
action ai_recycle_packet() {
  ...
  modify_field(pfuzz_visited.encoding_e0, 0);
}
...
action ai_turn_on_mutation() {
  ...
  modify_field(pfuzz_metadata.temp_port, 0);
}
...
table te_move_back_fields {
  ...
  actions {
    ai_NoAction;
    ai_drop_packet;
    ae_move_back_rewrite_mac;
    ae_move_back_fix_rewrite_mac;
  }
  default_action : ai_NoAction();
  size: 2048;
}

action ae_move_back_rewrite_mac(){
  modify_field(ipv4.dstAddr, pfuzz_metadata.max_bits_field0);
  modify_field(ipv4_clone.dstAddr, pfuzz_metadata.max_bits_field0);
}
action ae_move_back_fix_rewrite_mac(ipv4_dstAddr){
