...
register forward_count_ingress {
  width: 32;
  instance_count: 1;
}
blackbox stateful_alu forward_count_register_ingress {
    reg: forward_count_ingress;
    output_value: register_lo;
    update_lo_1_value: register_lo + 1;
}
action ai_count_ingress() {
    forward_count_register_ingress.execute_stateful_alu(0);
}
table ti_count_ingress{
  actions {
    ai_count_ingress;
  }
  default_action: ai_count_ingress();
  size: 0;
}

control ingress {
  apply(ti_count_ingress);
  if (fp4_visited.pkt_type == 1) {
...
field_list fi_bf_hash_fields_16 {
  fp4_visited.aeGetDiff;
  fp4_visited.aeReadMirror;
  fp4_visited.aeReadOriginal;
  fp4_visited.aeUpdateMirror;
  fp4_visited.aeUpdateOriginal;
  fp4_visited.aiSendClone;
  fp4_visited.aiSetOutputPort;
}
...
blackbox stateful_alu ri_bloom_filter_2_alu_update {
    ...
    output_value: alu_lo;    output_dst: fp4_metadata.reg_val_two;    update_lo_1_value: set_bit;
}
...
table ti_path_assertion {
  reads {
    ...
    fp4_visited.assertion_one : exact;
    fp4_visited.assertion_two : exact;
  }
  actions {
...
action ai_recycle_packet() {
  ...
  modify_field(fp4_visited.aeGetDiff, 0);
  modify_field(fp4_visited.aeReadMirror, 0);
  modify_field(fp4_visited.aeReadOriginal, 0);
  modify_field(fp4_visited.aeUpdateMirror, 0);
  modify_field(fp4_visited.aeUpdateOriginal, 0);
  modify_field(fp4_visited.aiSendClone, 0);
  modify_field(fp4_visited.aiSetOutputPort, 0);
  modify_field(fp4_visited.count_original, 0);
  modify_field(fp4_visited.count_mirror, 0);
  modify_field(fp4_visited.count_diff1, 0);
  modify_field(fp4_visited.count_diff2, 0);
}
...
table ti_create_packet {
  ...
  default_action : ai_add_ethernet();
  size: 256;
}
...
action ae_move_tiSendClone(){
  modify_field(fp4_metadata.max_bits_field0, ethernet.etherType);
}
...
action ae_apply_mutations0() {
  modify_field_rng_uniform(fp4_metadata.max_bits_field0, 0,0xffff);
}
...
table te_move_back_fields {
  reads {
    fp4_metadata.seed_num: exact;
    fp4_metadata.table_seed: exact;
    fp4_metadata.fixed_header_seed: exact;
  }
  actions {
    ai_NoAction;
    ai_drop_packet;
...
action ae_move_back_tiSendClone(){
  modify_field(ethernet.etherType, fp4_metadata.max_bits_field0);
  modify_field(ethernet_clone.etherType, fp4_metadata.max_bits_field0);
}
