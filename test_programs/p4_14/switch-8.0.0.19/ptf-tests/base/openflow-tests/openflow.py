...
sys.path.append(os.path.join(sys.path[0], '..', '..', '..', '..', 'testutils'))

sys.path.append(
    os.path.join(sys.path[0], '..', '..', '..', '..', 'programs', 'switch',
                 'openflow_mapping'))

sys.path.append(
    os.path.join(sys.path[0], '..', '..', '..', '..', 'targets',
                 'single_device', 'ptf-tests', 'pd_thrift'))
...
def get_match(match_fields):
    """
    Returns a packet and an OXM list that the packet matches,according to match_fields.
    """
...
def get_apply_actions(actions):
    """
    Returns a 1 element list of APPLY_ACTIONS instructions,with actions specified in actions.
    """
...
def get_group_all(gid, action_sets):
    ...
    for b in action_sets:
        buckets.append(
            ofp.bucket(actions=[get_action(a, arg) for a, arg in b.items()]))
...
def setup_default_table_configurations(client, sess_hdl, dev_tgt):
    ...
    mbr_hdl = client.bd_action_profile_add_member_with_set_bd(sess_hdl, dev_tgt,
                                                              action_spec)
    match_spec = dc_port_vlan_mapping_match_spec_t(
        ...
        vlan_tag__1__vid=0)
    client.port_vlan_mapping_add_entry(sess_hdl, dev_tgt, match_spec, mbr_hdl)
...
def setup_pre(mc, sess_hdl, dev_tgt):
    ...
    mc.mc_associate_node(sess_hdl, dev_tgt.dev_id, mgrp_hdl, node_hdl, 0, 0)
...
class Output(openflow_base_tests.OFTestInterface):
    ...
    def runTest(self):
        ...
        exp_pkt = simple_tcp_packet(
            dl_vlan_enable=True, vlan_vid=TEST_VLAN, eth_dst=TEST_ETH_DST)
...
class NWTTL(openflow_base_tests.OFTestInterface):
    ...
    def runTest(self):
        ...
        nw = {"OUTPUT": out_port, "SET_NW_TTL": ttl}
...
class GroupAdd(openflow_base_tests.OFTestInterface):
    ...
    def runTest(self):
        ...
        outport1, outport2 = ports[1], ports[2]
        ...
        exp_pkt1 = simple_tcp_packet(
            dl_vlan_enable=True, vlan_vid=10, eth_dst=TEST_ETH_DST)
        exp_pkt2 = simple_tcp_packet(
            dl_vlan_enable=True, vlan_vid=19, eth_dst=TEST_ETH_DST)
...
class GroupMod(openflow_base_tests.OFTestInterface):
    ...
    def runTest(self):
        ...
        exp_pkt1 = simple_tcp_packet(
            dl_vlan_enable=True, vlan_vid=6, eth_dst=TEST_ETH_DST)
        exp_pkt2 = simple_tcp_packet(
            dl_vlan_enable=True, vlan_vid=4, eth_dst=TEST_ETH_DST)
        ...
        bucket1 = {"SET_NW_TTL": 7, "OUTPUT": outport1}

        bucket2 = {"SET_NW_TTL": 17, "OUTPUT": outport2}

        bucket3 = {"SET_NW_TTL": 27, "OUTPUT": outport3}
...
class TableStatsGet(openflow_base_tests.OFTestInterface):
    ...
    def runTest(self):
        ...
        miss_pkt = str(
            simple_tcp_packet(
                eth_dst="00:77:22:55:99:11", dl_vlan_enable=True, vlan_vid=3))
...
class PacketIn(openflow_base_tests.OFTestInterface):
    ...
    def runTest(self):
        ...
        output = {"OUTPUT": ofp.const.OFPP_CONTROLLER}
