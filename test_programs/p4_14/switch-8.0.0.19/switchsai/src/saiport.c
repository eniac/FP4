...
sai_status_t sai_set_port_attribute(_In_ sai_object_id_t port_id,
  ...
  sai_port_bind_mode_t bind_mode = 0;
  switch_port_bind_mode_t switch_bind_mode = 0;
  ...
  switch (attr->id) {
    ...
    case SAI_PORT_ATTR_QOS_DEFAULT_TC:
      ...
      break;
    case SAI_PORT_ATTR_INGRESS_FILTERING:  // Unsupported
    ...
    case SAI_PORT_ATTR_QOS_PFC_PRIORITY_TO_PRIORITY_GROUP_MAP:
      switch_status = switch_api_port_pfc_priority_to_ppg_set(
          device, port_id, attr->value.oid);
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR(
    ...
    case SAI_PORT_ATTR_BIND_MODE:
      bind_mode = attr->value.u32;
      if (bind_mode == SAI_PORT_BIND_MODE_PORT) {
        switch_bind_mode = SWITCH_PORT_BIND_MODE_PORT;
      } else if (bind_mode == SAI_PORT_BIND_MODE_SUB_PORT) {
        switch_bind_mode = SWITCH_PORT_BIND_MODE_PORT_VLAN;
      } else {
        status = SAI_STATUS_INVALID_PARAMETER;
        SAI_LOG_ERROR("failed to set binding moded for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
      ...
      switch_status =
          switch_api_port_bind_mode_set(device, port_id, switch_bind_mode);
      status = sai_switch_status_to_sai_status(switch_status);
      if (status != SAI_STATUS_SUCCESS) {
        SAI_LOG_ERROR("failed to set binding mode for port %d: %s",
                      (port_id & 0xFFFF),
                      sai_status_to_string(status));
        return status;
      }
...
sai_status_t sai_get_port_attribute(_In_ sai_object_id_t port_id,
  ...
  for (i = 0, attr = attr_list; i < attr_count; i++, attr++) {
    switch (attr->id) {
      ...
      case SAI_PORT_ATTR_QOS_PFC_PRIORITY_TO_PRIORITY_GROUP_MAP:
        switch_status = switch_api_port_pfc_priority_to_ppg_get(
...
sai_status_t sai_get_port_stats(_In_ sai_object_id_t port_id,
  ...
  switch_uint64_t switch_counters[number_of_counters];
