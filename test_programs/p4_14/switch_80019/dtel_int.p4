...
control process_dtel_int_upstream_report {
    ...
    // remove all INT-related headers in the last INT control block in ingress
    if (valid(int_header)){
...
/*******************************************************************************
 ...
 If sink, original pkt
    if going out of a sink port and is also source (1hop sink), set sink=1
...
/*******************************************************************************
 Egress control block process_dtel_local_report_ that is broken into two blocks
    ...
    At sink, if digest is enabled (not report all) find if there is any flow state change
    At source/sink generate local report through e2e according to digest & queue report
 ******************************************************************************/

control process_dtel_int_sink_ports {
#ifdef INT_EP_ENABLE
...
/******************************************************************************
 control block process_dtel_insert_ that is broken into two smaller blocks
 1) at transit updates INT meta header, stack and digest
 2) at source and sink e2e adds INT meta header and stack
 3) at source adds INT flow state digest
 4) at source and transit updates INT encapsulation headers (ex: IP/UDP)
 ******************************************************************************/
...
control process_dtel_insert_2_ {
#ifdef INT_TRANSIT_ENABLE
