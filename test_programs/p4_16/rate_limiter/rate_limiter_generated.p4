...
@name(".reg_bit_counter_egress") Register<bit<8>, bit<32>>(32w2) reg_bit_counter_egress;

@name(".reg_interval_wma_egress") Register<bit<64>, bit<32>>(32w2) reg_interval_wma_egress;

@name(".reg_last_timestamp_egress") Register<bit<64>, bit<32>>(32w2) reg_last_timestamp_egress;
...
control egress(inout headers hdr, inout metadata meta, in egress_intrinsic_metadata_t eg_intr_md, in egress_intrinsic_metadata_from_parser_t eg_intr_md_from_parser_aux, inout egress_intrinsic_metadata_for_deparser_t eg_intr_md_for_dprsr, inout egress_intrinsic_metadata_for_output_port_t eg_intr_md_for_oport) {
    @name(".reBitCounter") RegisterAction<bit<8>, bit<32>, bit<8>>(reg_bit_counter_egress) reBitCounter = {
    ...
    @name(".reWmaPhase1") RegisterAction<reWmaPhase1_layout, bit<32>, bit<32>>(reg_last_timestamp_egress) reWmaPhase1 = {
        void apply(inout reWmaPhase1_layout value, out bit<32> rv) {
            rv = 32w0;
            reWmaPhase1_layout in_value;
            in_value = value;
            value.hi = meta.meta.cur_ts - in_value.lo;
            value.lo = meta.meta.cur_ts;
            rv = value.hi;
        }
    };
    @name(".reWmaPhase2_math_unit_0") MathUnit<bit<32>>(false, 2s0, -6s4, { 8w15, 8w14, 8w13, 8w12, 8w11, 8w10, 8w9, 8w8, 8w7, 8w6, 8w5, 8w4, 8w3, 8w2, 8w1, 8w0 }) reWmaPhase2_math_unit_0;
    @name(".reWmaPhase2") RegisterAction<reWmaPhase2_layout, bit<32>, bit<32>>(reg_interval_wma_egress) reWmaPhase2 = {
        void apply(inout reWmaPhase2_layout value, out bit<32> rv) {
            rv = 32w0;
            reWmaPhase2_layout in_value;
            in_value = value;
            if (meta.meta.bitCounter == 1w0) {
                value.hi = in_value.lo;
            }
            if (meta.meta.bitCounter == 1w0) {
                value.lo = in_value.lo + meta.meta.interArrival;
            }
            if (!(meta.meta.bitCounter == 1w0)) {
    ...
    @name(".aeInitializeWma") action aeInitializeWma() {
        meta.meta.cur_ts = (bit<32>)eg_intr_md_from_parser_aux.global_tstamp;
        meta.meta.bitCounter = (bit<1>)reBitCounter.execute((bit<32>)meta.ig_intr_md_for_tm.ucast_egress_port);
    }
    @name(".aeWmaPhase1") action aeWmaPhase1() {
        meta.meta.interArrival = reWmaPhase1.execute((bit<32>)meta.ig_intr_md_for_tm.ucast_egress_port);
    }
    @name(".aeWmaPhase2") action aeWmaPhase2() {
        meta.meta.current_reading = reWmaPhase2.execute((bit<32>)meta.ig_intr_md_for_tm.ucast_egress_port);
    }
    @name(".drop_packet") action drop_packet() {
        eg_intr_md_for_dprsr.drop_ctl = 3w1;
    }
    @name(".ae_rate_limit") action ae_rate_limit(bit<32> x) {
        meta.meta.current_reading = x |-| meta.meta.current_reading;
    }
    @name(".teInitializeWma") table teInitializeWma {
        actions = {
            aeInitializeWma();
        }
        size = 1;
    ...
    @name(".teWmaPhase1") table teWmaPhase1 {
        actions = {
            aeWmaPhase1();
        }
        size = 1;
    ...
    @name(".teWmaPhase2") table teWmaPhase2 {
        actions = {
            aeWmaPhase2();
        }
        size = 1;
    ...
    @name(".te_rate_limit") table te_rate_limit {
        actions = {
            ae_rate_limit();
        }
        size = 1;
    ...
    apply {
        ...
        teWmaPhase2.apply();
        te_rate_limit.apply();
        if (meta.meta.current_reading == 32w0) {
            te_drop.apply();
        }
    }
}
