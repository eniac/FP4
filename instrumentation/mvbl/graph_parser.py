...
class GraphParser(object):
    def __init__(self, prog_name, dotfile_ing, jsonfile, input_type='p414', direction='ingress'):
        ...
        logging.info(table_conditional_to_exit)
        ...
        for stage, table_list in stage2tables_dict.items():
            for table in table_list:
                if table not in table_graph.nodes:
                    logging.info("table: {} NOT in table_graph.nodes!".format(table))
                    raise Exception("ERR!")

        logging.info("\n====== logging.info roots and leaf_nodes ======")
        ...
        logging.info("\n====== Create full graph ======")
        logging.info("--- updated_nodes ---")
        logging.info(updated_nodes)
        logging.info("--- updated_edges ---")
        logging.info(updated_edges)
        ...
        for old_name, new_name in new_node_mapping.items():
            if old_name in table2actions_dict:
                if old_name != new_name:
                    logging.info("Unexpected rename of table old_name: {0} -> new_name: {1}".format(old_name, new_name))
        ...
        logging.info("--- table_conditional_to_exit after sanitization ---")
        logging.info(table_conditional_to_exit)
        ...
        logging.info("--- stage2tables_dict original ---")
            ...
            for i, table_or_condtional in enumerate(table_list):
                if table_or_condtional in new_node_mapping:
                    if table_or_condtional != new_node_mapping[table_or_condtional]:
                        logging.info("Must be a rename of conditional old_name: {0} -> new_name: {1}".format(table_or_condtional, new_node_mapping[table_or_condtional]))
                        table_list[i] = new_node_mapping[table_or_condtional]
                else:
                    logging.info("table_or_condtional {} not found in new_node_mapping!".format(table_or_condtional))
                    raise Exception("ERR!")
        logging.info("--- stage2tables_dict after ---")
        pretty_print_dict(stage2tables_dict)
        ...
        if cycles:
            logging.info("****** cycles found! ********")
            for cycle in cycles:
                logging.info(cycle)
            logging.info("****** end cycles found ********")
            logging.info("****** It should be a DAG ********")
            raise Exception("ERR!")
    ...
    def append_table_action_edges(self, table2actions_dict, edges, leaf_nodes, root_nodes, node2label_dict, actions2Table2nextTable_dict):
        logging.info("--- append_table_action_edges ---")

        full_graph_nodes = list(node2label_dict.values())
        logging.info("--- full_graph_nodes inherited from the original table_graph ---") 
        logging.info(full_graph_nodes)
            ...
            if e['src'] in table2actions_dict.keys():
                ...
                if e['dst'] in leaf_nodes and e['dst'] in table2actions_dict.keys():
                    for action in table2actions_dict[e['dst']]:
                        ...
                        logging.info("Append edge {}".format({'src': e['dst'], 'dst': action, 'label': ''}))
                        full_graph_nodes.append(action)
                        logging.info("Append node: {}".format(action))                        
        ...
        for de in edges_to_del:
            logging.info("de in edges_to_del: {}".format(de))
            if de in edges:
                logging.info("edges.remove: {}".format(de))
    ...
    def get_raw_nodes_from_dot(self, dotfile, cnt_blk='MyIngress'): # -> tuple[dict[str,str], dict[str,str]]:
        ...
        for subG in subgraphs:
            for n in subG.get_node_list():
                ...
                logging.info("--- name: {0}, label {1} ---".format(n.get_name(), label))
                if has_numbers(n.get_name()) and label not in ['__START__',"", '__EXIT__', 'tbl_act']:
                    ...
                    if label not in node_name_label.values():
                        logging.info("Write: {0}:{1}".format(n.get_name(), label))
                        node_name_label[n.get_name()] = label
                    else:
                        logging.info("TODO: Duplicate label, exit!!!")
                        raise Exception("ERR!")
                        cnt = 1
    ...
    def get_edges_from_dot(self, dotfile, nodes, ignored_nodes): # -> tuple[list[str], list[str]]:
        ...
        for subGraph in dot_graph.get_subgraphs():
            for e in subGraph.get_edge_list():
                # logging.info("--- src: {0}, dst: {1}, label: {2} ---".format(e.get_source(), e.get_destination(), e.obj_dict["attributes"]['label']))
        ...
        for e in original_src_dst_label:
            logging.info("--- src: {0}, dst: {1}, label: {2} ---".format(e['src'], e['dst'], e['label']))
            if e['src'] in nodes.keys() and e['dst'] in nodes.keys():
                logging.info("Append to renamed_src_dst_label: {}".format({'src':nodes[e['src']], 'dst':nodes[e['dst']], 'label': e['label']}))
                ...
                logging.info("[WARNING] Skipped {}".format({'src':nodes[e['src']], 'dst':ignored_nodes[e['dst']], 'label': e['label']}))
                if ignored_nodes[e['dst']] == "__EXIT__" or ignored_nodes[e['dst']] == "tbl_act":
                    ...
                    logging.info("[ERROR] Unexpected edge!")
                    raise Exception("ERR!")
            elif e['src'] not in nodes.keys() and e['dst'] in nodes.keys():
                logging.info("[WARNING] Skipped {}".format({'src':ignored_nodes[e['src']], 'dst':nodes[e['dst']], 'label': e['label']}))
                ...
                logging.info("[WARNING] Skipped {}".format({'src':ignored_nodes[e['src']], 'dst':ignored_nodes[e['dst']], 'label': e['label']}))
        ...
        logging.info("--- renamed_src_dst_label ---")
        pretty_print_dict(renamed_src_dst_label)
        logging.info("--- if_conditions_list ---")
    ...
    def fix_if_conditions_list(self, if_conditions_list):
        ...
        new_named_if_conditions = []
        for if_conditions in if_conditions_list:
            current_list = []
            for node in if_conditions:
                new_node = re.sub(r'\W+', '', node)
    ...
    def get_table_graph(self, node2label_dict, renamed_edges): # -> tuple[tuple[str,str,int], nx.DiGraph]:
        ...
        logging.info("add_nodes_from: {}".format(list(node2label_dict.values())))
    ...
    def extract_stages_p4_14(self, contextFile, node_labels, target_direction):
        ...
        for table_information in context_data['tables']:
            ...
            logging.info("--- name: {0}, table_type: {1}, direction: {2} ---".format(table_name, table_type, direction))
            is_matched = False
            ...
            if table_type == "condition":
                ...
                logging.info("raw condition: {0}, renamed: {1}".format(table_information['condition'], renamed_condition))
                for node_label in node_labels:
                    # logging.info("Try matching to node_label: {}".format(node_label))
                    if renamed_condition == node_label:
                        logging.info("[INFO] Exact matched to node_label: {}".format(node_label))
                        is_matched = True
            ...
            # Which is similar to a branch point
                ...
                if table_name == "tbl_act":
                    logging.info("[INFO] Skipped")
                for node_label in node_labels:
                    if table_name == node_label:
                        logging.info("Exact matched to node_label: {}".format(node_label))
                        is_matched = True
                        if label2matched[node_label] == 0:
                            label2matched[node_label] = 1
                        else:
                ...
                logging.info("Unknown table_type!")
                raise Exception("ERR!")
            if not is_matched:
                logging.info("[WARNING] Unmatched!")
        logging.info("--- Check if every nodes from dot are matched ---")
        for label in node_labels:
            if label2matched[label] == 0:
                raise Exception("{} unmatched!".format(label))

        logging.info("--- stage2tables_dict ---")
        pretty_print_dict(stage2tables_dict)
        logging.info("--- table2actions_dict ---")
        pretty_print_dict(table2actions_dict)

        logging.info("--- actions2Table2nextTable_dict ---")
        pretty_print_dict(actions2Table2nextTable_dict)
