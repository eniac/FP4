...
def encodePacketAndRuleList(packet, ruleList):
    # Packet: a bytearray
    # Rules: a list of nonempty strings which don't have the character '@'
    bytes = bytearray(b'@@').join([''] + ruleList)
    bytes += b"@$"
    bytes += packet
    return bytes
...
if __name__ == '__main__':
    ...
    packet1 = bytearray(b"\x00\x00\x00\x05\x39")
    packet2 = bytearray(b"asaaadf@@@@@@asdfas@@dfs")
    packet3 = bytearray(b"@")
    packet4 = bytearray(b"")
    ruleList1 = ["pd tiHandleOutgoingEthernet add_entry aiForward cis553_metadata_nextHop"]
    ruleList2 = ["pd tiHandleOutgoingEthernet add_entry", "pd tiHandleOutgoingEthernet add_entry", "pd tiHandleOutgoingEthernet add_entry"]
    ruleList3 = ["", ""]
    ruleList4 = []
    def test_packet_ruleList(packet, ruleList):
        enc = encodePacketAndRuleList(packet, ruleList)
        assert type(enc) is bytearray
        packet_, ruleList_ = decodePacketAndRuleList(enc)
        # print("Checking: {} {} {}".format(packet, ruleList, enc))
        assert type(packet) is bytearray
        assert type(packet_) is bytearray
        assert packet == packet_
        assert ruleList == ruleList_
        # print("Passed")
    test_packet_ruleList(packet1, ruleList1)
    test_packet_ruleList(packet2, ruleList2)
    test_packet_ruleList(packet3, ruleList3)
    test_packet_ruleList(packet4, ruleList4)
